...
const DEFAULTS = {
	models: {
		...
		fallback: {
			...
			provider: 'openrouter',
			modelId: 'mistralai/mistral-small-3.2-24b-instruct:free',
			maxTokens: 80000, // Default parameters if fallback IS configured
			temperature: 0.2
		}
	},
	global: {
	...
	for (const [provider, models] of Object.entries(MODEL_MAP)) {
		if (models.length > 0) {
			models.forEach((modelObj) => {
				...
				if (modelId === 'mistralai/mistral-small-3-2-24b:free')
					name = 'Mistral Small 3.2 24B (Free)';
				if (modelId === 'qwen/qwen3-14b:free')
					name = 'Qwen3 14B (Free)';
				if (modelId === 'qwen/qwen3-30b-a3b:free')
					name = 'Qwen3 30B MoE (Free)';
				if (modelId === 'qwen/qwen3-8b:free')
					name = 'Qwen3 8B (Free)';
				if (modelId === 'mistralai/devstral-small:free')
